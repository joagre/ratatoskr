...
typedef peg_context_t pcc_context_t;
...
static void pcc_action_Rule_0(peg_context_t *__pcc_ctx, pcc_thunk_t *__pcc_in, pcc_value_t *__pcc_out) {
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_Rule(pcc_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_Expression(pcc_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_Sequence(pcc_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_Prefix(pcc_context_t *ctx);
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_LEFTARROW(pcc_context_t *ctx);
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_QUESTION(pcc_context_t *ctx);
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_CLOSE(pcc_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_DOT(pcc_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_Spacing(pcc_context_t *ctx);
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_EndOfLine(pcc_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_EndOfFile(pcc_context_t *ctx);
static pcc_thunk_chunk_t *pcc_evaluate_rule_IdentStart(pcc_context_t *ctx);
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_Grammar(pcc_context_t *ctx) {
    ...
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_Spacing, &chunk->thunks, NULL)) goto L0000;
    {
        ...
        for (i = 0;; i++) {
            ...
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_Rule, &chunk->thunks, NULL)) goto L0001;
            if (ctx->cur == p) break;
            continue;
        L0001:;
            ctx->cur = p;
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_Rule(pcc_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "Rule", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    ...
    {
        pcc_thunk_t *const thunk = pcc_thunk__create_leaf(ctx->auxil, pcc_action_Rule_0, 1, 0);
    ...
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "Rule", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "Rule", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx, chunk);
    return NULL;
}
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_Expression(pcc_context_t *ctx) {
    ...
    ctx->level++;
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_Sequence, &chunk->thunks, NULL)) goto L0000;
    {
        int i;
        for (i = 0;; i++) {
            const size_t p = ctx->cur;
            const size_t n = chunk->thunks.len;
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_SLASH, &chunk->thunks, NULL)) goto L0001;
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_Sequence, &chunk->thunks, NULL)) goto L0001;
            if (ctx->cur == p) break;
            continue;
        L0001:;
            ctx->cur = p;
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_Sequence(pcc_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "Sequence", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    {
        int i;
        for (i = 0;; i++) {
    ...
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "Sequence", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
}
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_Prefix(pcc_context_t *ctx) {
    ...
    ctx->level++;
    {
        ...
        L0003:;
            ctx->cur = p;
            pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_NOT, &chunk->thunks, NULL)) goto L0004;
            goto L0002;
        L0004:;
            ctx->cur = p;
            pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
    ...
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_Suffix, &chunk->thunks, NULL)) goto L0000;
    ctx->level--;
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_Suffix(pcc_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "Suffix", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_Primary, &chunk->thunks, NULL)) goto L0000;
    {
        const size_t p = ctx->cur;
        const size_t n = chunk->thunks.len;
        {
            const size_t p = ctx->cur;
            const size_t n = chunk->thunks.len;
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_QUESTION, &chunk->thunks, NULL)) goto L0003;
            goto L0002;
        L0003:;
            ctx->cur = p;
            pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_STAR, &chunk->thunks, NULL)) goto L0004;
            goto L0002;
        L0004:;
            ctx->cur = p;
            pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_PLUS, &chunk->thunks, NULL)) goto L0005;
            goto L0002;
        L0005:;
            ctx->cur = p;
            pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
    ...
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "Suffix", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "Suffix", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx, chunk);
    return NULL;
}
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_EndOfLine(pcc_context_t *ctx) {
    pcc_thunk_chunk_t *const chunk = pcc_thunk_chunk__create(ctx);
    chunk->pos = ctx->cur;
    PCC_DEBUG(ctx->auxil, PCC_DBG_EVALUATE, "EndOfLine", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->buffer.len - chunk->pos));
    ctx->level++;
    {
        const size_t p = ctx->cur;
        const size_t n = chunk->thunks.len;
    ...
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_MATCH, "EndOfLine", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    return chunk;
L0000:;
    ctx->level--;
    PCC_DEBUG(ctx->auxil, PCC_DBG_NOMATCH, "EndOfLine", ctx->level, chunk->pos, (ctx->buffer.buf + chunk->pos), (ctx->cur - chunk->pos));
    pcc_thunk_chunk__destroy(ctx, chunk);
    return NULL;
}
