...
static pcc_thunk_chunk_t *pcc_evaluate_rule_MulExpr(pcc_context_t *ctx);
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_Expr(pcc_context_t *ctx) {
    ...
    ctx->level++;
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_AssignExpr, &chunk->thunks, NULL)) goto L0000;
    ctx->level--;
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_AndExpr(pcc_context_t *ctx) {
    ...
    ctx->level++;
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_MulExpr, &chunk->thunks, NULL)) goto L0000;
    {
        int i;
        for (i = 0;; i++) {
            const size_t p = ctx->cur;
            ...
            ctx->cur++;
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule__, &chunk->thunks, NULL)) goto L0001;
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_MulExpr, &chunk->thunks, NULL)) goto L0001;
            if (ctx->cur == p) break;
            continue;
        L0001:;
            ctx->cur = p;
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_MulExpr(pcc_context_t *ctx) {
    ...
    ctx->level++;
    if (!pcc_apply_rule(ctx, pcc_evaluate_rule_PostfixExpr, &chunk->thunks, NULL)) goto L0000;
    {
        int i;
        for (i = 0;; i++) {
            const size_t p = ctx->cur;
            ...
            ctx->cur++;
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule__, &chunk->thunks, NULL)) goto L0001;
            if (!pcc_apply_rule(ctx, pcc_evaluate_rule_PostfixExpr, &chunk->thunks, NULL)) goto L0001;
            if (ctx->cur == p) break;
            continue;
        L0001:;
            ctx->cur = p;
...
static pcc_thunk_chunk_t *pcc_evaluate_rule_PrimaryExpr(pcc_context_t *ctx) {
    ...
    L0009:;
        ctx->cur = p;
        pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
        if (!pcc_apply_rule(ctx, pcc_evaluate_rule_NumberLiteral, &chunk->thunks, NULL)) goto L0010;
        goto L0001;
    L0010:;
        ctx->cur = p;
        pcc_thunk_array__revert(ctx->auxil, &chunk->thunks, n);
