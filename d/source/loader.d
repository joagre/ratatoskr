...
struct Loader {
    ...
    public void prettyPrint() {
        uint byteIndex = 0;
        while (byteIndex < byteCode.length) {
            byteIndex += 1 + prettyPrintInstruction(&byteCode[byteIndex]);
        }
    }
    ...
    public uint prettyPrintInstruction(ubyte* bytes) {
        switch (bytes[0] >> 3) {
        case Opcodes.push:
            ...
            return long.sizeof;
        case Opcodes.pushs:
            auto length = get!ushort(&bytes[1]);
        ...
        case Opcodes.div:
            ...
            break;
        case Opcodes.jump:
            auto byteIndex = get!uint(&bytes[1]);
        ...
        case Opcodes.call:
            auto byteIndex = get!uint(&bytes[1]);
            auto arity = get!ubyte(&bytes[1 + uint.sizeof]);
