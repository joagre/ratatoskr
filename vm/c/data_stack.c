...
void data_stack_init(data_stack_t* data_stack) {
    dynarray_init(&data_stack->stack_array, NULL, 512, sizeof(uint8_t));
    data_stack->fp = 0;
}

void data_stack_free(data_stack_t* data_stack) {
    dynarray_clear(&data_stack->stack_array);
}

size_t data_stack_length(data_stack_t* data_stack) {
    return dynarray_size(&data_stack->stack_array);
}
...
vm_data_length_t data_stack_push(data_stack_t* data_stack, uint8_t* bytes,
    ...
    *data_address = dynarray_size(&data_stack->stack_array);
    for (size_t i = 0; i < length; i++) {
        dynarray_append(&data_stack->stack_array,
...
uint8_t* data_stack_peek(data_stack_t* data_stack, vm_address_t data_address) {
    return dynarray_element(&data_stack->stack_array, data_address);
}
